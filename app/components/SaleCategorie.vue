                                ...
                                <GridLayout v-for="(product, index) in leftColumnProducts"
                                    ...
                                    <StackLayout class="product-container">
                                        ...
                                        <GridLayout columns="auto, auto, *, auto"
                                                  ...
                                                  height="40">
                                            <Image :src="product.isFavorite ? '~/images/favoris-rempli.png' : '~/images/favoriteIcon.png'"
                                                   col="0"
                                                   class="action-icon"
    ...
    methods: {
        ...
        async toggleFavorite(product, index) {
            try {
                ...
                this.$set(this.products, index, { ...this.products[index], isFavorite });

                const response = await Http.request({
                    url: 'http://10.0.2.2:3000/Clients/add-favoris',
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": `Bearer ${token}`
                ...
                this.$set(this.products, index, { ...this.products[index], isFavorite: !isFavorite });
                alert({
                    title: "Erreur",
                    message: "Erreur lors de la mise à jour des favoris: " + (error.message || ""),
                    okButtonText: "OK"
                });
            }
        },
